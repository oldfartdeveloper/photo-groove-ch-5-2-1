...
viewFilter filterInfo =
    div [ class "filter-slider" ]
        ...
        , paperSlider
            ...
            []
        , label [] [ text (toString filterInfo.amount) ]
        ]
...
updateFilterInfo filter amount filterInfo =
    if filterInfo.type_ == filter then
        { filterInfo | amount = amount }
    else
        filterInfo
...
update msg model =
    case msg of
        SetFilter filter amount ->
            ( { model
                | filters =
                    List.map (updateFilterInfo filter amount) model.filters
              }
            , Cmd.none
            )
